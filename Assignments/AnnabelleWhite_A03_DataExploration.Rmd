...
bugs <- read.csv("Data/Raw/ECOTOX_Neonicotinoids_Insects_raw", stringsAsFactors = T)
trash <- read.csv("Data/Raw/NIWO_Litter/NEON_NIWO_Litter_massdata_2018-08_raw.csv", stringsAsFactors = T)

# I don't know why it is displaying a "cannot open the connection" error, because it keeps loading the data into R properly. I have been troubleshooting but haven't figured out what the issue is. As long as it correctly loads the data, I'm willing to tolerate a fraduluent error message. Would welcome insight.
```
...
```{r Species list}
sort(summary(bugs$Species.Common.Name))
```
...
ggplot(bugs) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) # Tilt columns so they're legible
```
...
# Fix data type
class(trash$collectDate) # Not a date; displays as factor (originally)
summary(trash$collectDate) # To show the date format; it's year-month-day
trash$collectDate <- ymd(trash$collectDate) # Perform Y/M/D function on the vector
class(trash$collectDate) # Check class; it's a date!

# Check for all unique dates:
unique(trash$collectDate)
# Looks like August 2, 16, and 30
```
...
ggplot(trash) +
  ...
  theme(axis.text.x = element_text(angle = 20, vjust = 0.5, hjust=1)) # I think it's easier to read this way
```
