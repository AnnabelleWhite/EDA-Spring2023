...
```{r DataExercise-1a}
highs <- c(71, 71, 64, 50, 43, 59, 60, 67)
lows <- c(49, 58, 47, 37, 35, 40, 49, 48)
```
...
```{r DataExercise-1c}
forecast <- data.frame(highs, lows, highC, lowC) #
names(forecast) <- c("High (F)","Low (F)","High (C)", "Low (C)"); View(dataframe1) #
```
...
```{r DataExercise-1d}
summary(forecast)
sd(forecast) # Doesn't work because it is processed as a list...
sd(forecast$`High (F)`)
```
